const setupCheck = (): void => {
  try {
    ...
    if (typeof apiKey === 'undefined') {
      throw new TypeError('❌ API 密钥未定义。')
    }
    if (typeof apiSecret === 'undefined') {
      throw new TypeError('❌ API 密钥未定义。')
    }
    if (typeof apiScopes === 'undefined') {
      throw new TypeError('❌ API 权限范围未定义。')
    }
    if (typeof appUrl === 'undefined') {
      throw new TypeError('❌ 应用 URL 未定义。')
    } else if (!appUrl.includes('https://')) {
      console.error('⚠️ 请为 SHOPIFY_APP_URL 使用 HTTPS。')
    }
    if (typeof apiVersion === 'undefined') {
      throw new TypeError('❌ API 版本未定义。')
    }
    if (typeof encString === 'undefined') {
      throw new TypeError('❌ 加密字符串未定义。')
    }

    if (typeof databaseURL === 'undefined') {
      throw new TypeError('❌ 数据库连接字符串未定义。')
    }
    ...
    if (typeof appHandle === 'undefined') {
      throw new TypeError('❌ 应用句柄未定义。')
    }
    if (appHandle.includes(' ')) {
      throw new Error('❌ 句柄必须进行 URL 编码且不能包含空格。')
    }
